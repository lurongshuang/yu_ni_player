...
class MediaKitEngine extends YuNiPlayerEngine {
  ...
  @override
  Future<void> performPlay() async => _player?.play();

  @override
  Future<void> performPause() async => _player?.pause();
  ...
  @override
  Future<void> setLoop(bool loop) async => _player?.setPlaylistMode(
  ...
  Future<void> setMute(bool mute) async =>
      _player?.setVolume(mute ? 0.0 : 100.0);

  @override
  Future<void> setRate(double rate) async => _player?.setRate(rate);
  ...
  void _subscribeToEvents() {
    ...
    _completedSub = p.stream.completed.listen((completed) {
      if (completed) updateState(YuNiPlayerState.completed);
    });
  ...
  Future<void> _cancelSubscriptions() async {
    await Future.wait([
      ...
      _bufferSub?.cancel() ?? Future.value(),
      _durationSub?.cancel() ?? Future.value(),
      _videoParamsSub?.cancel() ?? Future.value(),
      _playingSub?.cancel() ?? Future.value(),
      _bufferingSub?.cancel() ?? Future.value(),
      _completedSub?.cancel() ?? Future.value(),
      _errorSub?.cancel() ?? Future.value(),
    ]);
    _positionSub = _bufferSub = _durationSub = _videoParamsSub = null;
    _playingSub = _bufferingSub = _completedSub = _errorSub = null;
  }
}
