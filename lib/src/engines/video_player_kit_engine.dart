...
class VideoPlayerKitEngine extends YuNiPlayerEngine {
  ...
  Future<void> performRelease() async {
    ...
    if (c != null) {
      c.removeListener(_onControllerUpdate);
      _controller = null;
      // 先置 null 再 dispose，避免 _onControllerUpdate 在 dispose 期间
      // 访问已销毁的 controller
      await c.dispose();
    }
    // 不在 release 阶段预创建新 controller，等下次 init 时再创建，
    // 避免对象池回收期间异步 dispose 与新实例产生竞争
    _onPreparedCallback?.call(false);
    instanceCode.value = instanceCode.value + 1; // 通知 UI 层清除旧 View
  }
